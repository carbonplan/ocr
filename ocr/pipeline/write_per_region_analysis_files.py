...
def write_per_region(*, con: duckdb.DuckDBPyConnection, config: OCRConfig, region_type: RegionType):
    ...
    con.execute(f""" CREATE TEMP TABLE {region_type}_grouped_risk AS SELECT  b.NAME, b.GEOID,
                a.wind_risk_2011,
                a.wind_risk_2047,
                ...
                a.burn_probability_usfs_2047,  ST_X(ST_Centroid(a.geometry)) AS longitude,
    ST_Y(ST_Centroid(a.geometry)) AS latitude, a.geometry from read_parquet('{consolidated_buildings_uri.as_uri()}') a
    ...
    for geoid in geoid_list:
        ...
        (FORMAT CSV, COMPRESSION 'gzip', FILE_EXTENSION 'csv.gz', OVERWRITE_OR_IGNORE);""")

    # write geojson
    geojson_path = per_region_output_prefix / region_type / 'geojson'
    geojson_path.mkdir(parents=True, exist_ok=True)
    for geoid in geoid_list:
        con.execute(f"""COPY (SELECT * EXCLUDE latitude, longitude FROM {region_type}_grouped_risk WHERE GEOID = '{geoid}') TO '{geojson_path}/{geoid}.geojson'
        (FORMAT GDAL, DRIVER 'GEOJSON', LAYER_NAME {region_type}, OVERWRITE_OR_IGNORE);""")
