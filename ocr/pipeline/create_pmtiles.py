...
def create_pmtiles(branch: Branch):
    ...
    input_path = f"{s3_base}/intermediate/fire-risk/vector/{branch_value}/consolidated_geoparquet.parquet"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(
            ["s5cmd", "cp", "--sp", input_path, str(local_parquet)],
            check=True
        )
        ...
        subprocess.run([
            ...
            str(local_fgb), str(local_parquet),
        ...
        subprocess.run([
            "tippecanoe", "-o", str(local_pmtiles),
            "-l", "risk", "-n", "USFS BP Risk",
        ...
        subprocess.run(
            ["s5cmd", "cp", "--sp", str(local_pmtiles), output_path],
            check=True
        )
